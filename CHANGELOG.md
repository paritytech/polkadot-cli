...
- 26d5621: Add first-class Codex support to the agent skill, and a `dot skill` command to distribute it. The `dot-cli` SKILL.md was already format-compatible with Codex (same `name`/`description` frontmatter, same `references/` layout) — the gap was purely distribution, since Codex has no marketplace and instead scans `~/.agents/skills` (and repo `.agents/skills`).
...
- 6185ffb: Update all repository references from `peetzweg/polkadot-cli` to `paritytech/polkadot-cli` following the org move. This covers GitHub links and issue references, the codecov badge, the docs base URL, the plugin marketplace install command, and the marketplace owner metadata.
